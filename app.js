 ...
 * gigante aqui (isso causava um bug de cache/FUSE no ambiente de dev, ver histórico). */
...
const _assetDataUrlCache = {};
// Carrega um asset estático (logo, QR Pix padrão) como data URL, com cache
// em memória — só cacheia o SUCESSO: se o fetch falhar (ex: offline sem o
// asset em cache do Service Worker ainda), não fica preso nisso pelo resto
// da sessão, a próxima chamada tenta de novo em vez de cair no fallback pra
// sempre.
function loadAssetDataUrl(path) {
  if (_assetDataUrlCache[path]) return _assetDataUrlCache[path];
  const attempt = fetch(path)
    ...
    .catch(() => null);
  _assetDataUrlCache[path] = attempt.then((result) => {
    if (!result) delete _assetDataUrlCache[path];
    return result;
  });
  return _assetDataUrlCache[path];
}
// Logo da empresa, usada no cabeçalho do PDF (mesma posição da visualização
// da nota no app) — sem re-encode via canvas pra não perder transparência.
function getLogoDataUrl() { return loadAssetDataUrl('assets/logo.png'); }
// QR code Pix padrão, usado até o usuário subir outro em Configurações.
function getDefaultPixQrDataUrl() { return loadAssetDataUrl('assets/pix-qr-default.png'); }
async function buildNotaPdfDoc(v) {
  ...
  if (DEFAULT_PIX_KEY || state.pixQrCode) {
    const qrSize = 30;
    ...
    if (DEFAULT_PIX_KEY) {
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.text('Chave: ' + DEFAULT_PIX_KEY, marginX, pixTextY);
      pixTextY += 6;
    }
    if (state.pixQrCode) {
...
function saveDraftIfNeeded() {
  ...
  queueSync('service_draft_saved', draft);
...
function deleteDraft(id) {
  ...
  queueSync('service_draft_deleted', { id: d.id, client: d.client || null });
...
function notaCardHtml(v, forPrint) {
  ...
  return `<div id="${forPrint ? '' : 'nota-card'}" class="nota-card">
    ...
    ${(DEFAULT_PIX_KEY || state.pixQrCode) ? `<div class="nota-pix">
      <div class="nota-k">Pagamento via Pix</div>
      ${DEFAULT_PIX_KEY ? `<div class="nota-v mono" style="margin-top:3px;">${esc(DEFAULT_PIX_KEY)}</div>` : ''}
...
function configScreen() {
  ...
  return `<div class="screen">
    ...
    <label class="section-label">QR code do Pix</label>
    <div class="hint">Aparece na nota para o cliente pagar via Pix (a chave é fixa, definida pelo desenvolvedor).</div>
...
async function loadState() {
  ...
  state.syncQueue.forEach((item) => {
    if (item.synced_at === undefined && item.synced === true) {
      item.synced_at = item.created_at || new Date().toISOString();
      dbPut('syncQueue', item);
    }
  });
